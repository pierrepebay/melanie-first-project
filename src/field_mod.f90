...
contains
  ...
  subroutine initialize_field(field, cfg)
    ...
    do j = 1, cfg%ny
      ...
      time = end - start
      write(*, *) " Boucle initialize_field sans parallelisme : ", time
  ...
  subroutine diffuse_step(field, next_field, cfg)
    ...
    !$omp parallel default(none) shared(field, next_field) firstprivate(coefficient, cfg) private(i, j)
    ! print *, "Nombre de threads :", omp_get_num_threads()
